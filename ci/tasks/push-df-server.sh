...
cf create-service p-mysql 10mb df-mysql
 
#custom built SCDF server to enable PCF features
cf push dataflow-server -n df-mwright -m 2G -k 2G -b java_buildpack_offline -p spring-cloud-dataflow-server-cloudfoundry-1.2.0.BUILD-SNAPSHOT.jar --no-start
